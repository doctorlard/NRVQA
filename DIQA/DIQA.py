...
for epoch in range(10):
    ...
    for I_d, e_gt, r in train:
        ...
        _ = optimizer.apply_gradients(zip(gradients, objective_error_model.trainable_weights))
        _ = epoch_accuracy(e_gt, objective_error_model(I_d))

        if step % 100 == 0:
            print("epoch:%s step:%s loss= %s" % (epoch, step, epoch_accuracy.result().numpy()))
        step += 1
...
subjective_error_model = tf.keras.Model(input, h, name='subjective_error_model')
subjective_error_model.compile(
    optimizer=optimizer, loss=tf.losses.MeanSquaredError(), metrics=[tf.metrics.MeanSquaredError()]
)
